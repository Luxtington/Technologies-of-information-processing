...
public abstract class Entity {
    ...
    public final void collectFields(Class<?> cl, StringBuilder sb) throws Exception {
        ...
        if (!cl.isAnnotationPresent(ToString.class) || (cl.isAnnotationPresent(ToString.class)
                && cl.getAnnotation(ToString.class).value() == ToStringOption.YES)){
            for (Field f : fields){
                f.setAccessible(true);
                if (!f.isAnnotationPresent(ToString.class) || (f.isAnnotationPresent(ToString.class)
